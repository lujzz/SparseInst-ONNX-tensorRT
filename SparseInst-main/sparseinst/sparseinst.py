...
class SparseInst(nn.Module):
    ...
    def forward(self, images):
        ...
        if self.training:
            ...
            results = self.inference(output, images)
            ...
            out_pred_masks = torch.cat([r.pred_masks.unsqueeze(0) for r in results], dim=0)
    ...
    def inference(self, output, images):
        ...
        for _, (scores_per_image, mask_pred_per_image, image) in enumerate(zip(
            ...
            mask_pred_per_image = F.interpolate(
                mask_pred_per_image.unsqueeze(1), size=shape, mode="bilinear", align_corners=False)[:, :, :h, :w]
            mask_pred_per_image = F.interpolate(
                mask_pred_per_image, size=shape, mode='bilinear', align_corners=False).squeeze(1)
